...
Route::group(['middleware' => ['member', 'verified']], function () {
    ...
    // Express Interest
    Route::group(['middleware' => 'CheckUserData'], function () {
        // Your routes that need to check the introduction column
        ...
        // ShortList list, Chat, Ignore list, , member-listing
...
Route::post('/check-email', function (Request $request) {
    ...
    $message = $userExists ? 'Email already exists!' : null;

    return response()->json(['exists' => $userExists, 'message' => $message]);
})->name('check-email');
