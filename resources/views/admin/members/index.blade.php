            ...
            @can('create_member')
                <div class="col-md-6 text-right">
                    <a href="{{ route('members.create') }}"
                        class="btn btn-circle btn-primary">{{ translate('Add New Member') }}</a>
                </div>
            @endcan
        </div>
    </div>
                    ...
                    <table class="table aiz-table mb-0">
                        <thead>
                            <tr>
                                ...
                                @endif
                                <th data-breakpoints="md">{{ translate('Profile Updated') }}</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if (uploaded_asset($member->photo) != null)
                                            <img class="img-md" src="{{ uploaded_asset($member->photo) }}" height="45px"
                                                alt="{{ translate('photo') }}">
                                        @else
                                    ...
                                    @if (get_setting('member_approval_by_admin') == 1)
                                        <td>
                                            @if ($member->blocked == 1)
                                                <span
                                                    class="badge badge-inline badge-danger">{{ translate('Blocked') }}</span>
                                            @elseif($member->approved == 1)
                                                <span
                                                    class="badge badge-inline badge-success">{{ translate('Approved') }}</span>
                                            @else
                                                <span
                                                    class="badge badge-inline badge-pending">{{ translate('Pending') }}</span>
                                            @endif
                                        </td>
                                    @endif
                                    <td>
                                        ...
                                        @elseif($member->is_profile_updated == 1)
                                            <span
                                                class="badge badge-inline badge-success">{{ translate('Updated') }}</span>
                                        @else
                                            <span
                                                class="badge badge-inline badge-pending">{{ translate('Not Updated') }}</span>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        @if ($member->reported_users->count() > 0)
                                            <a href="@can('view_reported_profile'){{ route('reported_members', $member->id) }}@endcan"
                                                class="badge badge-inline badge-danger"
                                                title="{{ translate('View Reports') }}">{{ $member->reported_users->count() }}</a>
                                        @else
                                            0
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <span
                                                class="badge badge-inline badge-danger">{{ translate('Deactivated') }}</span>
                                        @endif
                                    </td>
                                    <td class="text-right">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    @can('view_member_profile')
                                                        <a class="dropdown-item"
                                                            href="{{ route('members.show', $member->id) }}">{{ translate('View') }}</a>
                                                    @endcan
                                                    @can('edit_member')
                                                        <a class="dropdown-item"
                                                            href="{{ route('members.edit', encrypt($member->id)) }}">{{ translate('Edit') }}</a>
                                                    @endcan
                                                    @can('block_member')
                                                        @if ($member->blocked == 0)
                                                            <a class="dropdown-item"
                                                                onclick="block_member({{ $member->id }})"
                                                                href="javascript:void(0);">{{ translate('Block') }}</a>
                                                        @elseif($member->blocked == 1)
                                                            <a class="dropdown-item"
                                                                onclick="unblock_member({{ $member->id }})"
                                                                href="javascript:void(0);">{{ translate('Unblock') }}</a>
                                                        @endif
                                                    @endcan
                                                    @can('approve_member')
                                                    ...
                                                    @can('update_member_package')
                                                        <a class="dropdown-item" onclick="package_info({{ $member->id }})"
                                                            href="javascript:void(0);">{{ translate('Package') }}</a>
                                                    @endcan
                                                    <a class="dropdown-item"
                                                        onclick="wallet_balance_update({{ $member->id }},{{ $member->balance }})"
                                                        href="javascript:void(0);">{{ translate('Wallet Balance') }}</a>

                                                    @can('login_as_member')
                                                        <a href="{{ route('members.login', encrypt($member->id)) }}"
                                                            class="dropdown-item">{{ translate('Log in as this Member') }}</a>
                                                    @endcan
                                                    @can('delete_member')
                                                        <a class="dropdown-item confirm-delete"
                                                            data-href="{{ route('members.destroy', $member->id) }}">{{ translate('Delete') }}</a>
                                                    @endcan
                                                </div>
                                            </div>
                                        </div>
...
@section('modal')
    ...
    <div class="modal fade member_wallet_balance_modal" id="modal-basic">
        <div class="modal-dialog">
            <div class="modal-content">
                <form class="form-horizontal member-block" action="{{ route('member.wallet_balance_update') }}"
                    method="POST">
                    @csrf
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-8">
                                <input type="number" class="form-control mb-3" id="member_wallet_balance"
                                    value="" readonly>
                            </div>
                        </div>
                        <div class="row">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-8">
                                <input type="number" lang="en" class="form-control mb-3" name="wallet_amount"
