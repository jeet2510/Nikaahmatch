<footer class="aiz-footer fs-13 mt-auto text-white fw-400 pt-5">
    <div class="container">
        ...
        @if (get_setting('footer_address') != null ||
                get_setting('footer_website') != null ||
                get_setting('footer_email') != null ||
                get_setting('footer_phones') != null)
            <div class="mb-4">
                <h4 class="text-uppercase text-primary fs-14 border-bottom border-primary pb-4 mb-4">
                    {{ translate('Contacts') }}</h4>
                <div class="row opacity-60 no-gutters">
                    <div class="col-xl col-md-6 mb-4">
                        ...
                        <div class="mb-3 opacity-60 mt-4">
                    ...
                    <div class="col-xl col-md-6 mb-4">
                        <div class="mb-3 opacity-60 ml-3">
                            {{-- <i class="las la-envelope mr-2"></i> --}}
                            <span>{{ translate('Support By') }}</span>
                        </div>
                        <div style="max-width: 100%; height: auto;">
                            <img src="/assets/img/support_by_footer_image.png" alt="Footer Image"
        ...
        <div class="row no-gutters">
            @if (!empty(get_setting('widget_one_labels')))
                <div class="col-xl col-md-6 mb-4">
                    <h4 class="text-uppercase text-primary fs-14 border-bottom border-primary pb-4 mb-4">
                        {{ get_setting('widget_one_title') }}</h4>
                    <div>
                            ...
                            @foreach (json_decode(get_setting('widget_one_labels'), true) as $key => $value)
                                <li class="my-3">
                                    <a href="{{ json_decode(get_setting('widget_one_links'), true)[$key] }}"
                                        class="text-reset opacity-60">{{ $value }}</a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
            ...
            @if (!empty(get_setting('widget_two_labels')))
                <div class="col-xl col-md-6 mb-4">
                    <h4 class="text-uppercase text-primary fs-14 border-bottom border-primary pb-4 mb-4">
                        {{ get_setting('widget_two_title') }}</h4>
                    <div>
                        <ul class="list-unstyled">
            ...
            @if (!empty(get_setting('widget_three_labels')))
                <div class="col-xl col-md-6 mb-4">
                    <h4 class="text-uppercase text-primary fs-14 border-bottom border-primary pb-4 mb-4">
                        {{ get_setting('widget_three_title') }}</h4>
                    <div>
                        <ul class="list-unstyled">
            ...
            @if (!empty(get_setting('widget_mobile_app_title')))
                <div class="col-xl col-md-6 mb-4">
                    <h4 class="text-uppercase text-primary fs-14 border-bottom border-primary pb-4 mb-4">
                        {{ get_setting('widget_mobile_app_title') }}</h4>
                    <div class="mb-3">
    ...
    <div class="row align-items-center gutters-5 text-center">
        <div class="col">
            <a href="{{ route('home') }}" class="text-reset d-block flex-grow-1 text-center py-2">
                ...
                <span
                    class="d-block fs-10 opacity-60 {{ areActiveRoutes(['home'], 'opacity-100 fw-600') }}">{{ translate('Home') }}</span>
            </a>
        </div>
        <div class="col">
            <a href="{{ route('frontend.notifications') }}" class="text-reset d-block flex-grow-1 text-center py-2">
                <span class="d-inline-block position-relative px-2">
                    <i
                        class="las la-bell fs-18 opacity-60 {{ areActiveRoutes(['frontend.notifications'], 'opacity-100') }}"></i>
                    @if (Auth::check() && Auth::user()->user_type == 'member')
                        ...
                        @if ($unseen_notification > 0)
                            <span
                                class="badge badge-sm badge-circle badge-primary position-absolute absolute-top-right">{{ $unseen_notification }}</span>
                        @endif
                    @endif
                </span>
                <span
                    class="d-block fs-10 opacity-60 {{ areActiveRoutes(['frontend.notifications'], 'opacity-100 fw-600') }}">{{ translate('Notifications') }}</span>
            </a>
        </div>
        <div class="col">
            <a href="{{ route('all.messages') }}"
                class="text-reset d-block flex-grow-1 text-center py-2 {{ areActiveRoutes(['all.messages'], 'opacity-100') }}">
                <span class="d-inline-block position-relative px-2">
                    <i
                        class="las la-comment-dots fs-18 opacity-60 {{ areActiveRoutes(['all.messages'], 'opacity-100') }}"></i>
                    @if (Auth::check() && Auth::user()->user_type == 'member')
                        @php
                        ...
                        @if ($unseen_chat_thread_count > 0)
                            <span
                                class="badge badge-sm badge-circle badge-primary position-absolute absolute-top-right">{{ $unseen_chat_thread_count }}</span>
                        @endif
                    @endif
                </span>
                <span
                    class="d-block fs-10 opacity-60 {{ areActiveRoutes(['all.messages'], 'opacity-100 fw-600') }}">{{ translate('Messages') }}</span>
            </a>
        </div>
        @if (Auth::check())
                ...
                <div class="col">
                    <a href="javascript:void(0)"
                        ...
                        <span class="d-block mx-auto mb-1 opacity-60">
                            <img src="{{ uploaded_asset(Auth::user()->photo) }}" class="rounded-circle size-20px"
        ...
        @else
            <div class="col">
                <a href="{{ route('login') }}" class="text-reset d-block flex-grow-1 text-center py-2">
                    ...
                    <span
                        class="d-block fs-10 opacity-60 {{ areActiveRoutes(['login'], 'opacity-100 fw-600') }}">{{ translate('Account') }}</span>
                </a>
            </div>
        @endif
    </div>
